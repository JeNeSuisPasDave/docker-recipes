  ...
  class Application < Sinatra::Base
    ...
    helpers do
      def get_war(name, url)
        s = 'docker run --rm --volumes-from dv_tomcat7_webapps --name "#{name}" jamtur01/fetcher "#{url}" 2>&1'
        puts s
        cid = `docker run --rm --volumes-from dv_tomcat7_webapps --name "#{name}" jamtur01/fetcher "#{url}" 2>&1`.chop
        puts cid
        [$?.exitstatus == 0, cid]
      end

      def create_instance(name)
        cid = `docker run -P --volumes-from dv_tomcat7_webapps -d -t jamtur01/tomcat7 2>&1`.chop
        [$?.exitstatus == 0, cid]
      end
